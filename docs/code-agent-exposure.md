...
| Host Wayland display socket | `/run/agentbox/wayland.sock` | Unix socket bind mount | Launches with usable host `WAYLAND_DISPLAY` |
...
If host `WAYLAND_DISPLAY` resolves to a usable Unix socket, `agentbox` bind-mounts only that socket at `/run/agentbox/wayland.sock` and sets `WAYLAND_DISPLAY` to that absolute container path. Relative host display names are resolved below host `XDG_RUNTIME_DIR`; absolute display paths are used directly. Invalid or inaccessible values produce a warning and do not block launch.

The container can communicate with the compositor using protocols available to the invoking host user. `agentbox` does not expose the containing host runtime directory, X11, D-Bus, PipeWire, desktop portals, or GPU devices as part of this passthrough. Existing containers retain the socket selected at launch and must be relaunched if the host replaces it.

## Environment

`agentbox` does not request whole-host environment passthrough for the container. The container receives image-defined environment such as `HOME=/home/user`, `NIX_REMOTE=daemon`, and `NIX_DAEMON_SOCKET_PATH=/nix/var/nix/daemon-socket/socket`, plus explicit launch environment entries generated by `agentbox` for runtime defaults, Codex `CODEX_HOME` passthrough, Git identity, Git excludes file passthrough, SSH agent passthrough, SSH host verification, and Wayland passthrough. Podman may still apply its own defaults, such as proxy environment propagation, according to the host Podman configuration.
