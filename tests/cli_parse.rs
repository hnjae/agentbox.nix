...
#[test]
fn config_subcommand_is_not_supported() {
    let bare_error = Cli::try_parse_from(["agentbox", "config"]).unwrap_err();
    let init_error = Cli::try_parse_from(["agentbox", "config", "init"]).unwrap_err();

    assert_eq!(bare_error.exit_code(), 2);
    assert_eq!(init_error.exit_code(), 2);
    assert!(
        bare_error
            .to_string()
            .contains("unrecognized subcommand 'config'")
    );
    assert!(
        init_error
            .to_string()
